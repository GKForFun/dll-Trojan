...
/*
	destroy some molloc()
*/
void destory_http(struct http_response* hresp)
{
...
struct http_response* handle_redirect_get(struct http_response* hresp, char* custom_headers)
{
	if(hresp->status_code_int > 300 && hresp->status_code_int < 399)
	{
		...
		while(token != NULL)
		{
			...
			{
				/* Extract url */
				char *location = str_replace((char *)"Location: ", (char *)"", token);
...
struct http_response* http_req(char *http_headers, struct parsed_url *purl)
{
	...
	*/
	...
	while((recived_len = recv(sock, BUF, BUFSIZ-1, 0)) > 0)
	{
        BUF[recived_len] = '\0';
#ifdef DEBUG
		response_tmp = (char*)calloc(strlen(response) + strlen(BUF) + 1, sizeof(char));
		sprintf(response_tmp, "%s%s", response, BUF);
		free(response);
		response = (char *)calloc(strlen(response_tmp), sizeof(char));
		memcpy(response, response_tmp, strlen(response_tmp));
		free(response_tmp);
#else
	...
	char *status_line = get_until(response, (char *)"\r\n");
	status_line = str_replace((char *)"HTTP/1.0 ", (char *)"", status_line);
	char *status_code = str_ndup(status_line, 4);
	status_code = str_replace((char *)" ", (char *)"", status_code);
	char *status_text = str_replace(status_code, (char *)"", status_line);
	status_text = str_replace((char *)" ", (char *)"", status_text);
	hresp->status_code = status_code;
	hresp->status_code_int = atoi(status_code);
	hresp->status_text = status_text;

	/* Parse response headers */
	char *headers = get_until(response, (char *)"\r\n\r\n");
...
struct http_response* http_post(char *url, char *custom_headers, char *post_data)
{
	...
	if (int len = strlen(http_headers) + strlen(post_data) > 1024)
	{
		if (len >= 8192)
			exit(1);
		http_headers = (char *)realloc(http_headers, strlen(http_headers) + strlen(post_data));
	}
	if(custom_headers != NULL)
	{
		sprintf(http_headers, "%s%s\r\n", http_headers, custom_headers);
