...
extern "C"
{
	...
	__declspec(dllexport)  HMEMORYMODULE MemoryLoadLibrary(const void * Data, size_t Size);

	__declspec(dllexport)  void MemoryFreeLibrary(HMEMORYMODULE Module);

	__declspec(dllexport) FARPROC MemoryGetProcAddress(HMEMORYMODULE Module, LPCSTR lpProcName);

	__declspec(dllexport) HMODULE FileLoadLibrary(LPCTSTR filePath);

	__declspec(dllexport) void FileFreeLibrary(HMODULE hModule);

	__declspec(dllexport) FARPROC FileGetProcAddress(HMODULE Module, LPCSTR lpProcName);
}
